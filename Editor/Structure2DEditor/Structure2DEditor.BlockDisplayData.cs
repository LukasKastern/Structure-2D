...
namespace Structure2D
{
    ...
    {
        private class BlockDisplayData
        {
            ...
            public BlockDisplayData(Block block, string Path, bool isActive)
            {
                IsActive = isActive;
                _name = block.name;
                this.Path = Path;
            ...
            private void DrawContent(Rect viewRect)
            {
                ...
                if (CanDrawContent)
                {
                    Block.IsSolid = EditorGUILayout.Toggle("Is Solid", Block.IsSolid);

                    var currentBlockAmount = Block.LightBlockAmount / 255f;
                    ...
                    var blockAmount = EditorGUILayout.Slider("Light block amount", currentBlockAmount, minLightBlockAmount, 1);

                    Block.LightBlockAmount = (byte)(blockAmount * 255f);
                    
                    Block.Texture = (Texture2D)EditorGUILayout.ObjectField("Texture", Block.Texture, typeof(Texture2D), false, GUILayout.MaxWidth(210));
            ...
            private string GetErrorMessage()
            {
                ...
                if (Block.Texture == null)
                    return "Block has no texture assigned to it";
            
                else if (Block.Texture.width != TextureDimensions.x || Block.Texture.height != TextureDimensions.y)
                    return
                        $"Block Texture doesn't have the correct size, the size of the given texture is {new Vector2Int(Block.Texture.width, Block.Texture.height)} but the set size is {TextureDimensions}";
                else
                    return "";
            ...
            {
                if (_name == Block.name) return;
                
                AssetDatabase.RenameAsset(Path, _name);

                Path = AssetDatabase.GetAssetPath(Block);
            }
        }
    }
}
